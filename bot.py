...
def social_status_text(user):
    ...
    return (
        ...
        + "\n\n".join(lines)
        + f"\n\nApproved: {claimed_count}/{len(SOCIALS)}"
        + f"\nPending: {pending_count}"
        + "\n\n📸 After submitting, send screenshot proof in the group or to an admin. Admin approval is required before XP is added."
    )
...
def submit_social(message, key):
    ...
    bot.reply_to(
        ...
        f"User ID: {user.id}\n"
        f"Platform: {data['label']}\n"
        f"Reward after approval: +{SOCIAL_XP_PER_FOLLOW} XP\n\n"
        "📸 Please send screenshot proof.\n"
        f"Admins approve with: /{data['approve']} {user.id}"
    )
...
def approve_social(message, key):
    ...
    cur.execute(f"SELECT xp, {data['claimed']}, {data['pending']} FROM users WHERE user_id=?", (user_id,))
    row = cur.fetchone()
    ...
    cur.execute(f"""
    UPDATE users
    SET xp=?, {data['claimed']}=1, {data['pending']}=0
    WHERE user_id=?
    """, (xp, user_id))
    conn.commit()
    ...
    try:
        bot.send_message(
            user_id,
            f"🎉 Your {data['label']} follow was approved!\n+{SOCIAL_XP_PER_FOLLOW} XP added.\nTotal XP: {xp}"
        )
    except Exception:
        pass
...
def reject_social(message, key):
    ...
    bot.reply_to(
        message,
        f"❌ Rejected {data['label']} social claim for {get_user_display(user_id)}."
    )

def pending_socials_text():
    rows_text = ["⏳ Pending Social Approvals\n"]
    ...
    for key, data in SOCIALS.items():
        ...
        FROM users
        WHERE {data['pending']}=1 AND {data['claimed']}=0
        """)
        rows = cur.fetchall()
    ...
    if not found:
        return "✅ No pending social approvals."
...
def start(message):
    ...
    bot.reply_to(
        ...
        "Earn XP by chatting, inviting members, and completing approved social tasks.\n\n"
        "Commands:\n"
        "/rank — view your XP\n"
        "/top — view leaderboard\n"
        "/invite — get your invite link\n"
        "/socials — view social tasks\n"
...
def submit_x(message):
    submit_social(message, "x")

@bot.message_handler(commands=["submitlinkedin"])
def submit_linkedin(message):
    submit_social(message, "linkedin")

@bot.message_handler(commands=["submitinstagram"])
def submit_instagram(message):
    submit_social(message, "instagram")

@bot.message_handler(commands=["submitwhatsapp"])
def submit_whatsapp(message):
    submit_social(message, "whatsapp")

@bot.message_handler(commands=["submittiktok"])
def submit_tiktok(message):
    submit_social(message, "tiktok")

@bot.message_handler(commands=["submityoutube"])
def submit_youtube(message):
    submit_social(message, "youtube")

@bot.message_handler(commands=["submitfacebook"])
def submit_facebook(message):
    submit_social(message, "facebook")

# Approval commands
@bot.message_handler(commands=["approvex"])
def approve_x(message):
    approve_social(message, "x")

@bot.message_handler(commands=["approvelinkedin"])
def approve_linkedin(message):
    approve_social(message, "linkedin")

@bot.message_handler(commands=["approveinstagram"])
def approve_instagram(message):
    approve_social(message, "instagram")

@bot.message_handler(commands=["approvewhatsapp"])
def approve_whatsapp(message):
    approve_social(message, "whatsapp")

@bot.message_handler(commands=["approvetiktok"])
def approve_tiktok(message):
    approve_social(message, "tiktok")

@bot.message_handler(commands=["approveyoutube"])
def approve_youtube(message):
    approve_social(message, "youtube")

@bot.message_handler(commands=["approvefacebook"])
def approve_facebook(message):
    approve_social(message, "facebook")

# Rejection commands
@bot.message_handler(commands=["rejectx"])
def reject_x(message):
    reject_social(message, "x")

@bot.message_handler(commands=["rejectlinkedin"])
def reject_linkedin(message):
    reject_social(message, "linkedin")

@bot.message_handler(commands=["rejectinstagram"])
def reject_instagram(message):
    reject_social(message, "instagram")

@bot.message_handler(commands=["rejectwhatsapp"])
def reject_whatsapp(message):
    reject_social(message, "whatsapp")

@bot.message_handler(commands=["rejecttiktok"])
def reject_tiktok(message):
    reject_social(message, "tiktok")

@bot.message_handler(commands=["rejectyoutube"])
def reject_youtube(message):
    reject_social(message, "youtube")

@bot.message_handler(commands=["rejectfacebook"])
def reject_facebook(message):
    reject_social(message, "facebook")
...
def main():
    ...
    print(f"🌐 Social reward: {SOCIAL_XP_PER_FOLLOW} XP per approved platform")
